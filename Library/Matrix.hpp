...
//		arm_mat{M, N, &data[0][0]}
	...
	MatrixBase(const int *d)// :
//		arm_mat{M, N, &data[0][0]}
	{
		memcpy(data, d, sizeof(data));
	}

	MatrixBase(const int d[M][N])// : 
//		arm_mat{M, N, &data[0][0]}
	{
		memcpy(data, d, sizeof(data));
	}
	 ...
	 * access by index
	 */
	int operator()(const unsigned int row, const unsigned int col) const {
		return data[row][col];
	}
	 ...
	 * uniform scaling
	 */
	Matrix<M, N> operator *(const int num) const {
		Matrix<M, N> res;
	...
	Matrix<M, N> &operator *=(const int num) {
		for (unsigned int i = 0; i < M; i++)
			for (unsigned int j = 0; j < N; j++)
				data[i][j] *= num;
...
public:
	...
	Matrix(const int *d) : MatrixBase<M, N>(d) {}

	Matrix(const int d[M][N]) : MatrixBase<M, N>(d) {}
...
public:
	...
	Matrix(const int *d) : MatrixBase<3, 3>(d) {}

	Matrix(const int d[3][3]) : MatrixBase<3, 3>(d) {}
	...
	Matrix<3, 3> operator *(const Matrix<3, 3> &m) const {
		...
		for(i=0;i<3;i++){
			for(j=0;j<3;j++){
				res.data[i][j] = data[i][0] * m.data[0][j] + data[i][1] * m.data[1][j] + data[i][2] * m.data[2][j];
			}
		}	
		return res;
	}
...
/*	void from_euler(float roll, float pitch, float yaw) {
	...
	void anti_sym(const Vector<3> &v) {
		data[0][0] = 0;
		data[0][1] = -v.data[2];
		data[0][2] = v.data[1];
		data[1][0] = v.data[2];
...
public:
	...
	Matrix<3, 1> operator *(const Matrix<3, 1> &v) const {
		Matrix<3, 1> res;
		res(0,0)=data[0][0] * v.data[0][0] + data[0][1] * v.data[1][0] + data[0][2] * v.data[2][0];
		res(1,0)=data[1][0] * v.data[0][0] + data[1][1] * v.data[1][0] + data[1][2] * v.data[2][0];
		res(2,0)=data[2][0] * v.data[0][0] + data[2][1] * v.data[1][0] + data[2][2] * v.data[2][0];
		return res;
	}
	/**
	 * create a rotation matrix from given euler angles
